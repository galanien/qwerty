...
function createUser(username, password) {
    ...
    fetch(apiUrl, {
        ...
        .then(data => {
            ...
            displayMessage('Sign up successful.', 'signupMessage');
        })
        .catch(error => {
            displayMessage('Sign-up failed. Please retry.', 'signupMessage');
...
function signUp() {
    ...
    checkUsernameExists(username)
        .then(usernameExists => {
            if (usernameExists) {
                displayMessage('Failed to check if the username is already in use.', 'signupMessage');
            } else {
    ...
    fetch(`${apiUrl}?username=${username}&password=${password}`)
        ...
        .then(data => {
            if (data.length > 0) {
                ...
                localStorage.setItem('userId', userId);
                displayMessage('Sign in successful!', 'signinMessage');
                setTimeout(() => {
